...
def read_in():
    ...
    if request.method == 'POST':
        ...
        if not os.path.isdir('static'):
            os.mkdir('static')
        filepath=os.path.join('static',"profile.pdf")
        file.save(filepath)
...
def profile():
    ...
    with open('static/profile.pdf', 'rb') as in_file:
    ...
    cred_object = firebase_admin.credentials.Certificate('firebaseadminkeypython.json')
    ...
    doc_ref = db.collection(u'user').document("1")
    doc_ref.set({
        u'fname': fname,
        u'lname': lname,
        u'email': email_var,
    ...
    firebase_admin.initialize_app(cred_object, {'databaseURL':'https://console.firebase.google.com/u/0/project/winlysisproject/firestore/data/~2FUser'}, name="certificates-record")
    db = firestore.client()
    ...
    for c in certifications:
        ...
        doc_ref = db.collection(u'certificates-record').document("cert-"+str(n))
        doc_ref.set({
    ...
    # Employment Record
    default_app = firebase_admin.initialize_app(cred_object, {'databaseURL':'https://console.firebase.google.com/u/0/project/winlysisproject/firestore/data/~2FUser'}, name = "employment_record")
    db = firestore.client()
    ...
    for e in experience:
        ...
        n = random.randint(0,10)
        doc_ref = db.collection(u'employment_record').document("emp-"+str(n))
        doc_ref.set({
    ...
    firebase_admin.initialize_app(cred_object, {'databaseURL':'https://console.firebase.google.com/u/0/project/winlysisproject/firestore/data/~2FUser'}, name="education_record")
    db = firestore.client()
    ...
    for ed in education:
        ...
        n = random.randint(0,10)
        doc_ref = db.collection(u'education_record').document("edu-"+str(n))
        doc_ref.set({
    ...
    print(uid)
    return render_template('index.html', text=id)
